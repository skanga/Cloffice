...
const RISK_CONFIG = {
  low: { color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-950/30', border: 'border-emerald-200 dark:border-emerald-800/50', label: 'Niedrig', badgeVariant: 'default' as const },
  medium: { color: 'text-amber-600', bg: 'bg-amber-50 dark:bg-amber-950/30', border: 'border-amber-200 dark:border-amber-800/50', label: 'Mittel', badgeVariant: 'outline' as const },
  high: { color: 'text-orange-600', bg: 'bg-orange-50 dark:bg-orange-950/30', border: 'border-orange-200 dark:border-orange-800/50', label: 'Hoch', badgeVariant: 'outline' as const },
  critical: { color: 'text-red-600', bg: 'bg-red-50 dark:bg-red-950/30', border: 'border-red-200 dark:border-red-800/50', label: 'Kritisch', badgeVariant: 'outline' as const },
} as const;

const SAFETY_STORAGE_KEY = 'relay.safety.scopes';

const DEFAULT_SCOPES: PermissionScope[] = [
  { id: 'file-read', name: 'Dateien lesen', description: 'Agent kann Dateien im Arbeitsordner lesen', riskLevel: 'low', enabled: true, requiresApproval: false },
  { id: 'file-list', name: 'Ordner auflisten', description: 'Agent kann Ordnerinhalte auflisten', riskLevel: 'low', enabled: true, requiresApproval: false },
  { id: 'file-create', name: 'Dateien erstellen', description: 'Agent kann neue Dateien im Arbeitsordner erstellen', riskLevel: 'medium', enabled: true, requiresApproval: true },
  { id: 'file-modify', name: 'Dateien ändern', description: 'Agent kann bestehende Dateien modifizieren', riskLevel: 'medium', enabled: true, requiresApproval: true },
  { id: 'file-delete', name: 'Dateien löschen', description: 'Agent kann Dateien dauerhaft entfernen', riskLevel: 'high', enabled: false, requiresApproval: true },
  { id: 'file-move', name: 'Dateien verschieben', description: 'Agent kann Dateien in andere Ordner verschieben', riskLevel: 'medium', enabled: true, requiresApproval: true },
  { id: 'network-request', name: 'Netzwerk-Anfragen', description: 'Agent kann HTTP-Anfragen an externe APIs senden', riskLevel: 'high', enabled: false, requiresApproval: true },
  { id: 'shell-execute', name: 'Shell-Befehle', description: 'Agent kann Terminalbefehle ausführen', riskLevel: 'critical', enabled: false, requiresApproval: true },
  { id: 'email-send', name: 'E-Mails senden', description: 'Agent kann E-Mails über konfigurierte Konnektoren senden', riskLevel: 'high', enabled: false, requiresApproval: true },
  { id: 'calendar-modify', name: 'Kalender ändern', description: 'Agent kann Kalendereinträge erstellen oder ändern', riskLevel: 'medium', enabled: false, requiresApproval: true },
  { id: 'data-export', name: 'Daten exportieren', description: 'Agent kann Daten aus der App exportieren', riskLevel: 'medium', enabled: true, requiresApproval: false },
  { id: 'memory-write', name: 'Memory schreiben', description: 'Agent kann dauerhaft Informationen speichern', riskLevel: 'low', enabled: true, requiresApproval: false },
];
    ...
    <section className="mx-auto grid h-full w-full max-w-[1060px] min-h-0 grid-rows-[auto_auto_minmax(0,1fr)] gap-3">
      ...
      <header>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-semibold tracking-tight">Sicherheit & Genehmigungen</h1>
        </div>
        <p className="mt-1 font-sans text-sm text-muted-foreground">
          Steuere, was dein Agent tun darf. Aktiviere Genehmigungspflichten für risikoreiche Aktionen.
        </p>
      </header>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-border/60 bg-card px-4 py-2.5">
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-sans text-[12px] text-muted-foreground">{stats.enabled} Aktiv</span>
        </div>
        <div className="flex items-center gap-1.5">
          <Lock className="size-3.5 text-amber-500" />
          <span className="font-sans text-[12px] text-muted-foreground">{stats.withApproval} mit Genehmigung</span>
        </div>
        <Separator orientation="vertical" className="h-4" />
        {stats.highRisk > 0 && (
          <div className="flex items-center gap-1.5">
            <ShieldAlert className="size-3.5 text-red-500" />
            <span className="font-sans text-[12px] text-red-600">{stats.highRisk} hohes Risiko aktiv</span>
          </div>
        )}
        {pendingCount > 0 && (
          <>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-sans text-[12px] font-medium text-amber-600">{pendingCount} ausstehend</span>
            </div>
          </>
        )}
      </div>
          ...
          <div className="flex items-center gap-2 border-b border-border/40 px-4 py-2">
            ...
            <div className="flex items-center gap-1">
              {(['all', 'low', 'medium', 'high', 'critical'] as const).map((level) => (
                ...
                >
                  {level === 'all' ? 'Alle' : RISK_CONFIG[level].label}
                </button>
              ))}
            </div>
          </div>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div className="ml-auto flex items-center gap-1">
                                <button
                                  ...
                                  title={scope.enabled ? 'Deaktivieren' : 'Aktivieren'}
                                >
                                  {scope.enabled ? (
                                ...
                                <div className="mt-2 flex items-center gap-3">
                                  <label className="flex cursor-pointer items-center gap-1.5">
                                    ...
                                    <span className="font-sans text-[11px] text-muted-foreground">
                                      Genehmigung erforderlich (Human-in-the-Loop)
                                    </span>
                                  </label>
                                </div>
                              </div>
          ...
          <div className="flex min-h-0 flex-col rounded-xl border border-amber-200 bg-amber-50/30 dark:border-amber-800/40 dark:bg-amber-950/20">
            <div className="border-b border-amber-200/60 px-3 py-2.5 dark:border-amber-800/30">
              ...
              <p className="mt-0.5 font-sans text-[11px] text-muted-foreground">
                Dein Agent wartet auf Genehmigung.
              </p>
            </div>
            <ScrollArea className="flex-1">
                    ...
                    return (
                      <div key={action.id} className="rounded-lg border border-border/60 bg-white p-2.5 dark:bg-background">
                        ...
                        <div className="mt-2 flex items-center gap-1.5">
                          ...
                          <button type="button" className="ml-auto" title="Vorschau">
