...
function titleCase(value: string): string {
  if (!value.trim()) return 'Unbekannt';
...
function getRelativeTimeLabel(dateStr: string | null): string {
  ...
  if (absDiff < 60_000) return diff > 0 ? 'Gleich' : 'Gerade eben';
  if (absDiff < 3_600_000) {
    const mins = Math.floor(absDiff / 60_000);
    return diff > 0 ? `in ${mins} Min.` : `vor ${mins} Min.`;
  }
  if (absDiff < 86_400_000) {
    const hrs = Math.floor(absDiff / 3_600_000);
    return diff > 0 ? `in ${hrs} Std.` : `vor ${hrs} Std.`;
  }
    ...
    <section className="mx-auto grid h-full w-full max-w-[1060px] min-h-0 grid-rows-[auto_auto_minmax(0,1fr)] gap-3">
      ...
      <header className="flex items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-semibold tracking-tight">Zeitplan</h1>
            <Badge variant="outline" className="ml-2 font-sans text-[11px]">
              {jobs.length} {jobs.length === 1 ? 'Job' : 'Jobs'}
            </Badge>
          </div>
          <p className="mt-1 font-sans text-sm text-muted-foreground">
            Alle geplanten Aufgaben deines OpenClaw-Gateways im Überblick.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex rounded-lg border border-border">
            {(['timeline', 'calendar'] as const).map((mode) => (
              ...
              >
                {mode === 'timeline' ? 'Timeline' : 'Kalender'}
              </button>
            ))}
          </div>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-border/60 bg-card px-4 py-2.5">
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-sans text-[12px] text-muted-foreground">{enabledCount} Aktiv</span>
        </div>
        <div className="flex items-center gap-1.5">
          <Pause className="size-3.5 text-muted-foreground/50" />
          <span className="font-sans text-[12px] text-muted-foreground">{disabledCount} Pausiert</span>
        </div>
        ...
        {upcomingJobs.length > 0 && (
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-sans text-[12px] text-muted-foreground">
              Nächster: {getRelativeTimeLabel(upcomingJobs[0].nextRunAt)}
            </span>
          </div>
        )}
        <span className="ml-auto font-sans text-[11px] text-muted-foreground/60">
          {status.trim() || 'Verbunden.'}
        </span>
      </div>
            ...
            {sortedJobs.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="font-sans text-sm text-muted-foreground">
                  Keine geplanten Jobs. Stelle sicher, dass dein Gateway konfiguriert ist.
                </p>
              </div>
            ) : (
                      ...
                      <div key={job.id} className="relative flex gap-3 py-1.5">
                        ...
                        <div className="min-w-0 flex-1 rounded-lg border border-border/60 bg-background px-3 py-2.5">
                          ...
                          <div className="mt-1.5 flex gap-4 font-sans text-[11px] text-muted-foreground">
                            <span>Nächster Lauf: {formatTime(job.nextRunAt)}</span>
                            <span>Letzter Lauf: {formatTime(job.lastRunAt)}</span>
                          </div>
                        </div>
                      </div>
                    );
                ...
                <ScrollArea className="flex-1">
                  {selectedDateJobs.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-8 text-center">
                      ...
                      <p className="font-sans text-[12px] text-muted-foreground">Keine Jobs an diesem Tag.</p>
                    </div>
                  ) : (
